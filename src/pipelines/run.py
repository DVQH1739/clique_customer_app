...
def pipeline_retail(xlsx: str | None = None, steps: set[str] | None = None) -> None:
    ...
    if "train" in steps:
        ...
        if prep is None:
            ...
            X_train = pd.read_csv(config.X_TRAIN_SCALED_CSV)[
    ...
    if "benchmark" in steps:
        ...
        if cluster_desc is None:
            ...
            cluster_desc = joblib.load(config.PROFILES_PKL)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="CLIQUE clustering on Online Retail II (xlsx)"
